IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'TF' AND name = 'MGetHieuChuanKeGSTT')
   exec('CREATE FUNCTION  dbo.MGetHieuChuanKeGSTT()returns  @MAY_NGAY_CUOI TABLE ([MS_MAY] [nvarchar](30) NOT NULL) as begin return end')
GO

 --  SELECT *FROM [dbo].[MGetHieuChuanKeGSTT]('01/01/2019','12/31/2019','admin','-1',-1,-1,'-1','-1',-1,1,0) ORDER BY MS_MAY,MS_BO_PHAN,MS_TS_GSTT,MS_TT,MS_DV_TG,NGAY_KE
...
SELECT DISTINCT MS_MAY,TEN_MAY FROM [dbo].[MGetMayUserNgay](@NgayBD,@UserName,@MsNXuong,@MsHeThong,@MsBPCP,@LoaiMay,@NhomMay,'-1',0)
...
CASE @NNgu WHEN 0 THEN T5.TEN_DV_TG WHEN 1 THEN ISNULL(NULLIF(T5.TEN_DV_TG_ANH,''),T5.TEN_DV_TG) ELSE ISNULL(NULLIF(T5.TEN_DV_TG_HOA,''),T5.TEN_DV_TG) END  AS TEN_DV_TG,X.STT_CUOI
FROM 
(
 SELECT MS_MAY ,MS_BO_PHAN,SO_NGAY.MS_TS_GSTT,SO_NGAY.MS_TT,SO_NGAY.NGAY_CUOI , SO_NGAY.CHU_KY_DO, MS_DV_TG, SO_NGAY, 
 ...
 FROM 
  (
   SELECT     T1.MS_MAY,T1.MS_BO_PHAN, T1.MS_TS_GSTT,T1.MS_TT,T1.MS_DV_TG, T1.NGAY_CUOI , 
     T1.CHU_KY_DO,
...
) X INNER JOIN @MAY_USER T1 ON X.MS_MAY = T1.MS_MAY  INNER JOIN @HC_USER T2 ON T2.MS_TS_GSTT = X.MS_TS_GSTT 
...
IF @MLoai = 1
 BEGIN
  DECLARE @MS_MAY NVARCHAR(30),@TEN_MAY NVARCHAR(1000),@TEN_BO_PHAN  NVARCHAR(1000), @TEN_TS_GSTT NVARCHAR(1000), @MS_BO_PHAN NVARCHAR(50), @MS_TS_GSTT NVARCHAR(10),@MS_TT int,@NGAY_CUOI DATETIME,@CHU_KY_DO INT ,@MS_DV_TG INT,@NGAY_KE DATETIME,@TEN_DV_TG NVARCHAR(1000),@STT_CUOI int

  DECLARE BTDK_CURSOR CURSOR FOR
   SELECT DISTINCT MS_MAY,TEN_MAY, TEN_BO_PHAN,TEN_TS_GSTT,MS_BO_PHAN,MS_TS_GSTT,MS_TT,NGAY_CUOI,CHU_KY_DO ,MS_DV_TG,NGAY_KE,TEN_DV_TG,STT_CUOI FROM @MAY_NGAY_CUOI  
    WHERE MS_DV_TG IN (1,2,3,4) AND CHU_KY_DO IS NOT NULL AND CHU_KY_DO > 0
  OPEN BTDK_CURSOR
  FETCH NEXT FROM BTDK_CURSOR 
  INTO @MS_MAY,@TEN_MAY,@TEN_BO_PHAN,@TEN_TS_GSTT,@MS_BO_PHAN,@MS_TS_GSTT,@MS_TT,@NGAY_CUOI,@CHU_KY_DO ,@MS_DV_TG,@NGAY_KE,@TEN_DV_TG,@STT_CUOI
  ...
  BEGIN 
   ...
   WHILE @NGAY_KE <= @NgayKT 
    BEGIN 
     INSERT INTO @MAY_NGAY_CUOI (MS_MAY,TEN_MAY,TEN_BO_PHAN,TEN_TS_GSTT,MS_BO_PHAN,MS_TS_GSTT,MS_TT,NGAY_CUOI,CHU_KY_DO ,MS_DV_TG,NGAY_KE,TEN_DV_TG,STT_CUOI) 
     VALUES (@MS_MAY,@TEN_MAY,@TEN_BO_PHAN,@TEN_TS_GSTT,@MS_BO_PHAN,@MS_TS_GSTT,@MS_TT,@NGAY_CUOI,@CHU_KY_DO ,@MS_DV_TG,@NGAY_KE,@TEN_DV_TG,@STT_CUOI) 
