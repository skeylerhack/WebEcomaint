...
select ROW_NUMBER() OVER(ORDER BY a1.MS_DE_XUAT DESC) AS STT, case @NNGU when 0 then a4.TEN_PT when 1 then isnull(a4.TEN_PT_ANH,a4.Ten_PT) when 2 then isnull(a4.TEN_PT_HOA,a4.Ten_PT) end as TEN_PT, a4.QUY_CACH, NHA_CUNG_CAP, 
		...
		case BUYING_NEW when 0 then '' when 1 then 'x' END as BUYING_NEW, case REPLACING_FOR when 0 then '' when 1 then 'x' END as REPLACING_FOR, case SPARE when 0 then '' when 1 then 'x' END as SPARE, case MAINTENANCE when 0 then '' when 1 then 'x' END as MAINTENANCE,
		 DUONG_DAN_TL, NGAY_GIAO_HANG, t.HO + ' ' + t.TEN as BWHOM, TRANSFER_TO    from DE_XUAT_MUA_HANG_CHI_TIET a1 left join DON_VI_TINH a2 on a1.DVT = a2.TEN_1 join IC_PHU_TUNG a4 on a1.MS_PT = a4.MS_PT
