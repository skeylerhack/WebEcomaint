...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'spBCDanhSachVatTuSuDungBaoTri')
   exec('CREATE PROCEDURE spBCDanhSachVatTuSuDungBaoTri AS BEGIN SET NOCOUNT ON; END')
GO


--spBCDanhSachVatTuSuDungBaoTri '01/01/2015','01/01/2016','trungnv'

ALTER procedure spBCDanhSachVatTuSuDungBaoTri
	@TN DATETIME,
	@DN DATETIME, 
...
SELECT DISTINCT 
                         T1.MS_NHOM_MAY, T2.NGAY AS NGAY_XUAT, T7.MS_PHIEU_BAO_TRI, T6.NGAY_BD_KH, T3.MS_DH_XUAT_PT, T3.MS_PT, T5.TEN_PT, T3.SO_LUONG_THUC_XUAT, T9.DON_GIA, T9.NGOAI_TE, T9.TY_GIA, 
                         T10.MS_LOAI_VT, T10.TEN_LOAI_VT_TV, T11.MS_N_XUONG, T11.TEN_N_XUONG, T1.MS_LOAI_MAY, T1.TEN_LOAI_MAY, T1.MS_MAY, T1.TEN_MAY
