--them ngay cuoi
--exec [SP_Y_GET_TSGSTTDHKT_BAOCAO] 'administrator',0,'02/01/2018','02/28/2018',-1, -1,-1,'-1',-1,-1,-1,-1 
--exec [SP_Y_GET_TSGSTTDHKT_BAOCAO] 'administrator',0,'01/01/2019','12/31/2019',-1, -1,-1,'-1',-1 ,2
...
AS
    BEGIN 
        ...
        FROM    ( SELECT  DISTINCT
                            ...
                            T4.TEN_MAY ,
                            T1.MS_BO_PHAN + ' - '
                            + CASE @LANGUAGE WHEN 0 THEN T3.TEN_BO_PHAN WHEN 1 THEN ISNULL(NULLIF(T3.TEN_BO_PHAN_ANH,''),T3.TEN_BO_PHAN) ELSE ISNULL(NULLIF(T3.TEN_BO_PHAN_HOA,''),T3.TEN_BO_PHAN) END AS TEN_BO_PHAN,
                            T5.TEN_TS_GSTT ,
                            T2.MS_DV_TG ,
                  ...
                  FROM      dbo.CAU_TRUC_THIET_BI_TS_GSTT T1
                            INNER JOIN dbo.MGetHieuChuanKeGSTT(@TU_NGAY,
                                                              @DEN_NGAY,
                                                              @USERNAME,
                                                              @MS_NHA_XUONG,
                                                              @MS_HE_THONG,-1,
                                                              @MS_LOAI_MAY,
                                                              '-1',@MS_LOAI_CV, 1,@LANGUAGE) T2 ON T2.MS_MAY = T1.MS_MAY
...
(SELECT T1.Ten_N_XUONG AS MS_N_XUONG,T1.MS_MAY,T1.TEN_MAY,T1.TEN_BO_PHAN,T1.TEN_TS_GSTT,T1.NGAY_KT_KE, CONVERT(varchar, T1.NGAY_KT_KE, 103) AS NGAY,T1.CHU_KY_DO,T1.NGAY_KT_CUOI,T1.MS_DV_TG FROM #temptt T1)
 A
		PIVOT
		(
			COUNT(A.NGAY_KT_KE)
		...
		SELECT MS_N_XUONG ,MS_MAY,TEN_MAY,TEN_BO_PHAN,TEN_TS_GSTT,CHU_KY_DO,NGAY_KT_CUOI, ' + @sSql
                    + ',		
