...
--exec SP_Y_GET_TSGSTTDHKT 'admin',0,'01/01/2011','12/30/2018','-1',-1,'-1','-1','-1','-1','-1'
...
SELECT distinct
          ...
          T1.TEN_MAY,
           CTTB.TEN_BO_PHAN, 
           dbo.THONG_SO_GSTT.TEN_TS_GSTT, 
		   CONVERT(VARCHAR(8), CTTB_GSTT.CHU_KY_DO)            + ' '+ CASE @LANGUAGE WHEN 1 THEN dbo.DON_VI_THOI_GIAN.TEN_DV_TG_ANH WHEN 2 THEN dbo.DON_VI_THOI_GIAN.TEN_DV_TG_HOA
           ELSE dbo.DON_VI_THOI_GIAN.TEN_DV_TG END AS CHU_KY_DO, GSTT_TMP.NGAY_KT_CUOI, 
          dbo.GIA_TRI_TS_GSTT.TEN_GIA_TRI, dbo.GIA_TRI_TS_GSTT.DAT, 
          ...
          GSTT_TMP.NGAY_KT_CUOI) END AS NGAY_KT_KE,T1.MS_N_XUONG INTO #TEMP_DINH_TINH
FROM         dbo.CAU_TRUC_THIET_BI_TS_GSTT CTTB_GSTT INNER JOIN
      dbo.GIAM_SAT_TINH_TRANG_TS_DT GSTT_DT INNER JOIN
      #GIAM_SAT_TINH_TRANG_TS_TMP GSTT_TMP ON GSTT_DT.STT = GSTT_TMP.STT AND 
      GSTT_DT.MS_MAY = GSTT_TMP.MS_MAY AND 
      GSTT_DT.MS_TS_GSTT = GSTT_TMP.MS_TS_GSTT AND 
      GSTT_DT.MS_TT = GSTT_TMP.MS_TT AND 
      GSTT_DT.MS_BO_PHAN = GSTT_TMP.MS_BO_PHAN ON 
      CTTB_GSTT.MS_MAY = GSTT_TMP.MS_MAY AND 
      CTTB_GSTT.MS_BO_PHAN = GSTT_TMP.MS_BO_PHAN AND 
      CTTB_GSTT.MS_TS_GSTT = GSTT_TMP.MS_TS_GSTT INNER JOIN
      dbo.THONG_SO_GSTT ON CTTB_GSTT.MS_TS_GSTT = dbo.THONG_SO_GSTT.MS_TS_GSTT INNER JOIN
      dbo.GIA_TRI_TS_GSTT ON dbo.THONG_SO_GSTT.MS_TS_GSTT = dbo.GIA_TRI_TS_GSTT.MS_TS_GSTT AND 
      GSTT_DT.STT_GT = dbo.GIA_TRI_TS_GSTT.STT INNER JOIN
...
WHERE dbo.THONG_SO_GSTT.LOAI_TS = 1 AND CTTB_GSTT.ACTIVE=1
AND CASE CTTB_GSTT.MS_DV_TG 
	...
	WHEN 4 THEN  DATEADD(YEAR,CTTB_GSTT.CHU_KY_DO,GSTT_TMP.NGAY_KT_CUOI) END >= @TU_NGAY 
AND CASE CTTB_GSTT.MS_DV_TG 
	WHEN 1 THEN  DATEADD(DAY,CTTB_GSTT.CHU_KY_DO,GSTT_TMP.NGAY_KT_CUOI)
	WHEN 2 THEN  DATEADD(WEEK,CTTB_GSTT.CHU_KY_DO ,GSTT_TMP.NGAY_KT_CUOI)
	WHEN 3 THEN  DATEADD(MONTH,CTTB_GSTT.CHU_KY_DO,GSTT_TMP.NGAY_KT_CUOI)
...
TEMP.MS_MAY,TEMP.TEN_MAY,TEMP.TEN_BO_PHAN,TEMP.TEN_TS_GSTT,TEMP.CHU_KY_DO,TEMP.NGAY_KT_CUOI,
	   TEMP.TEN_GIA_TRI,ISNULL(TEMP.DAT,0) DAT,TEMP.NGAY_KT_KE
...
SELECT  DISTINCT
          CTTB.MS_MAY, 
          T1.TEN_MAY,
            CTTB.TEN_BO_PHAN  AS TEN_BO_PHAN, 
           ...
           ELSE dbo.DON_VI_THOI_GIAN.TEN_DV_TG END AS CHU_KY_DO, GSTT_TMP.NGAY_KT_CUOI, 
          (CTTB_GSTT.TEN_GT +' (' + CAST(GIAM_SAT_TINH_TRANG_TS.GIA_TRI_DO AS VARCHAR(10)) +')') as TEN_GIA_TRI, CTTB_GSTT.DAT, 
          CASE CTTB_GSTT.MS_DV_TG WHEN 1 THEN DATEADD(DAY, CTTB_GSTT.CHU_KY_DO, 
          GSTT_TMP.NGAY_KT_CUOI) WHEN 2 THEN DATEADD(DAY, 
          CTTB_GSTT.CHU_KY_DO * 7.25, GSTT_TMP.NGAY_KT_CUOI) 
          WHEN 3 THEN DATEADD(MONTH, CTTB_GSTT.CHU_KY_DO, 
...
WHERE dbo.THONG_SO_GSTT.LOAI_TS =0  AND CTTB_GSTT.ACTIVE=1
AND CASE CTTB_GSTT.MS_DV_TG 
	WHEN 1 THEN  DATEADD(DAY,CTTB_GSTT.CHU_KY_DO,GSTT_TMP.NGAY_KT_CUOI)
