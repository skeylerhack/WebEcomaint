...
--SELECT * FROM          dbo.MGetMayUserNgay('12/01/2016', 'ADMIN', '-1', 2, - 1, '-1', '-1', '-1', 0)
...
as 
begin
IF @UserName = '-1' SET @UserName = 'Admin'
--CASE @NNgu WHEN 0 THEN T1.TEN_LOAI_MAY WHEN 1 THEN ISNULL(NULLIF(T1.TEN_LOAI_MAY_ANH,''),T1.TEN_LOAI_MAY) ELSE ISNULL(NULLIF(T1.TEN_LOAI_MAY_HOA,''),T1.TEN_LOAI_MAY) END
...
INSERT INTO @MAY_TMP	
SELECT dbo.MAY.MS_MAY, MAY_NHA_XUONG_TMP.MS_N_XUONG, MS_HE_THONG, dbo.NHOM_MAY.MS_NHOM_MAY, 
		dbo.NHOM_MAY.MS_LOAI_MAY, A.MS_BP_CHIU_PHI, 
		TEN_N_XUONG, TEN_HE_THONG, 
		CASE @NNgu WHEN 0 THEN TEN_NHOM_MAY WHEN 1 THEN ISNULL(NULLIF(TEN_NHOM_MAY_ANH,''),TEN_NHOM_MAY) ELSE ISNULL(NULLIF(TEN_NHOM_MAY_HOA,''),TEN_NHOM_MAY) END  AS TEN_NHOM_MAY,
		CASE @NNgu WHEN 0 THEN TEN_LOAI_MAY WHEN 1 THEN ISNULL(NULLIF(TEN_LOAI_MAY_ANH,''),TEN_LOAI_MAY) ELSE ISNULL(NULLIF(TEN_LOAI_MAY_HOA,''),TEN_LOAI_MAY) END  AS TEN_LOAI_MAY,
		TEN_BP_CHIU_PHI
		
	FROM 
	(
	SELECT MS_MAY,MS_HE_THONG,TEN_HE_THONG FROM [dbo].[MGetUserMayHTCha](@MsHeThong,@UserName,@NgayHT,@NNgu)
	) AS MAY_HE_THONG_TMP 
		INNER JOIN
	(SELECT DISTINCT  A.MS_MAY, A.MS_N_XUONG,
		CASE @NNgu WHEN 0 THEN TEN_N_XUONG WHEN 1 THEN ISNULL(NULLIF(TEN_N_XUONG_A,''),TEN_N_XUONG) ELSE ISNULL(NULLIF(TEN_N_XUONG_H,''),TEN_N_XUONG) END  AS TEN_N_XUONG
	 FROM (
	...
	(SELECT MS_LOAI_MAY FROM dbo.MashjGetNhomLoaiMay(@UserName) AS MashjGetNhomLoaiMay_1) AS NHOM_LOAI_MAY ON 
		...
		INNER JOIN
	( SELECT DISTINCT MBPCP.MS_MAY, MBPCP.MS_BP_CHIU_PHI, 
		CASE @NNgu WHEN 0 THEN TEN_BP_CHIU_PHI WHEN 1 THEN ISNULL(NULLIF(TEN_BPCP_ANH,''),TEN_BP_CHIU_PHI) ELSE ISNULL(NULLIF(TEN_BPCP_HOA,''),TEN_BP_CHIU_PHI) END  AS TEN_BP_CHIU_PHI
	FROM dbo.MAY_BO_PHAN_CHIU_PHI AS MBPCP 
	...
	WHERE (A.MS_BP_CHIU_PHI = @MsBPCPhi OR @MsBPCPhi = -1) 
	...
	INSERT INTO @NLMTMP([MS_MAY],[MS_NHOM_MAY],MS_LOAI_MAY,[MS_HSX],[MS_HIEN_TRANG],[MS_KH],[SO_NAM_KHAU_HAO],[MO_TA],[NHIEM_VU_THIET_BI]
		...
		SELECT DISTINCT dbo.MAY.[MS_MAY],dbo.MAY.[MS_NHOM_MAY],MS_LOAI_MAY,[MS_HSX],[MS_HIEN_TRANG],[MS_KH],[SO_NAM_KHAU_HAO],[MO_TA],[NHIEM_VU_THIET_BI]
           ,[MODEL],[SERIAL_NUMBER],[NGAY_SX],[NUOC_SX],[NGAY_MUA],[NGAY_BD_BAO_HANH],[NGAY_DUA_VAO_SD]
           ,[SO_THE],[SO_NGAY_LV_TRONG_TUAN],[SO_GIO_LV_TRONG_NGAY],[MS_DVT_RT],[TY_LE_CON_LAI],[MUC_UU_TIEN]
           ,[SO_THANG_BH],[GIA_MUA],[NGOAI_TE],[LUU_Y_SU_DUNG],[Anh_TB],[CHU_KY_HC_TB],
		   CASE @NNgu WHEN 0 THEN TEN_MAY WHEN 1 THEN ISNULL(NULLIF(TEN_MAY_ANH,''),TEN_MAY) ELSE ISNULL(NULLIF(TEN_MAY_HOA,''),TEN_MAY) END  AS TEN_MAY
           ,[AN_TOAN],[MS_HO_SO],[TEN_HO_SO],[NGAY_HIEU_LUC_HO_SO],[SO_CT],[TI_GIA_VND]
           ...
           ,TEN_N_XUONG, TEN_HE_THONG, TEN_NHOM_MAY, TEN_LOAI_MAY
           , TEN_BP_CHIU_PHI, ISNULL([DINH_MUC_SL], 0), [DVT_SL]
