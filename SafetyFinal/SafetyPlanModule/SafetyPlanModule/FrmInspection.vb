...
Public Class FrmInspection
    ...
    Private Sub XtraForm1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ''CopyPictureToLocalPath()
        LoadListLocation()
        LoadStatus()
        LoadHPES()
        LoadListInCharge()
        cbThoigian.Text = Now.Year.ToString()
        'GetListData()
    ...
    Private Sub GridView1_CellValueChanged(ByVal sender As System.Object, ByVal e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles GridView1.CellValueChanged
        Try
            ...
            If GridView1.FocusedColumn.FieldName = "PathPicAfterDefault" Then
                ...
                End If
            End If
            If GridView1.FocusedColumn.FieldName = "Status" Then
    ...
    Private Sub GetListData()
        Try
            ...
            Try
                ...
                For i As Integer = 0 To GridView1.RowCount - 1
                    ...
                    ''If Not IsDBNull(GridView1.GetRowCellValue(i, "PathPicBeforeDefault")) And Not GridView1.GetRowCellValue(i, "PathPicBeforeDefault") Is Nothing Then
                    ''    If System.IO.File.Exists(Application.StartupPath + "/Images/" + GridView1.GetRowCellValue(i, "NamePicBeforeDefault")) Then

                    ''        Dim img As Image = System.Drawing.Image.FromFile(Application.StartupPath + "/Images/" + GridView1.GetRowCellValue(i, "NamePicBeforeDefault"))
                    ''        If img.Height > 60 Then
                    ''            Dim tyle As Double = img.Width / 60
                    ''            img = img.GetThumbnailImage(GridView1.Columns("PicBeforeDefault_IMG").Width + 64, 60, Nothing, New IntPtr())
                    ''        End If
                    ...
                    End If
                    ''If Not IsDBNull(GridView1.GetRowCellValue(i, "PathPicAfterDefault")) And Not GridView1.GetRowCellValue(i, "PathPicAfterDefault") Is Nothing Then
                    ''    If System.IO.File.Exists(Application.StartupPath + "/Images/" + GridView1.GetRowCellValue(i, "NamePicAfterDefault")) Then
                    ''        Dim img As Image = System.Drawing.Image.FromFile(Application.StartupPath + "/Images/" + GridView1.GetRowCellValue(i, "NamePicAfterDefault"))
                    ''        If img.Height > 60 Then
                    ''            Dim tyle As Double = img.Width / 60
                    ''            img = img.GetThumbnailImage(GridView1.Columns("PicBeforeDefault_IMG").Width + 64, 60, Nothing, New IntPtr())
                    ''        End If
        ...
        Try
            ...
            GridView1.Columns(7).MaxWidth = 75
            GridView1.Columns(7).MinWidth = 75

            GridView1.Columns(18).Width = 80
            GridView1.Columns(18).MaxWidth = 80
            GridView1.Columns(18).MinWidth = 80

            GridView1.Columns(19).Width = 100
            GridView1.Columns(19).MaxWidth = 100
            GridView1.Columns(19).MinWidth = 100
        Catch ex As Exception

        End Try
    End Sub

    Private Sub GetListDataByYear()
        Try
            tb = New DataTable
            GridView1.RowHeight = 60
            Dim tbl As New DataTable()

            tbl = GetListInspectionByYear().Copy
            tb = tbl.Clone
            tbl.Columns.Add("PicBeforeDefault_IMG", GetType(Image))
            tbl.Columns.Add("PicAfterDefault_IMG", GetType(Image))
            tbl.Columns("InCharge").ReadOnly = False
            tbl.Columns("ReportDate").ReadOnly = False
            GridControl1.DataSource = tbl
            Try
                If Not System.IO.Directory.Exists(DungChung.FolderShare) Then
                    Exit Sub
        ...
        Try
            ...
            GridView1.Columns(19).Width = 50
            GridView1.Columns(19).MaxWidth = 50
            GridView1.Columns(19).MinWidth = 50
        Catch ex As Exception
...
#Region "Get/Save data"
    ...
    Private Sub LoadHPES()
        ...
        Try
            ...
            GridView1.Columns("HPES").ColumnEdit = cboStatus

            Dim CheckedComboBoxEdit1 As New DevExpress.XtraEditors.Repository.RepositoryItemCheckedComboBoxEdit
            If dt.Rows.Count > 0 Then
                ...
                For ct As Integer = 0 To dt.Rows.Count - 1
                    ...
                    it.Description = dt.Rows(ct)(1).ToString()
                    it.Value = dt.Rows(ct)(0).ToString()
    ...
    Private Sub LoadStatus()
        ...
        Try
            ...
            r("NAME") = "N/A"
            dt.Rows.Add(r)
            Dim cboStatust As New DevExpress.XtraEditors.Repository.RepositoryItemLookUpEdit
            cboStatust.NullText = ""
            cboStatust.ValueMember = "ID"
            cboStatust.DisplayMember = "NAME"
            cboStatust.DataSource = dt

            cboStatust.Columns.Clear()
            cboStatust.Columns.Add(New DevExpress.XtraEditors.Controls.LookUpColumnInfo("ID"))
            cboStatust.Columns.Add(New DevExpress.XtraEditors.Controls.LookUpColumnInfo("NAME"))
            cboStatust.PopupWidth = 30
            cboStatust.Columns("ID").Width = 5
            cboStatust.Columns("NAME").Width = 25

            GridView1.Columns("Status").ColumnEdit = cboStatust

        Catch ex As Exception

        End Try
    End Sub
    Private Sub Combo_EditValueChanged(ByVal sender As Object, ByVal e As EventArgs)
    ...
    Public Function GetListInspection() As DataTable
        ...
        End Try
    End Function
    Public Function GetListInspectionByYear() As DataTable
        Try
            Dim dt As New DataTable()
            Dim sqlcom As New SqlCommand()
            Dim con As New SqlConnection(Commons.IConnections.ConnectionString())
            ...
            End If
            sqlcom.Connection = con
            sqlcom.Parameters.AddWithValue("ACTION", "LIST_BY_YEAR")
            sqlcom.Parameters.AddWithValue("YEAR", cbThoigian.Text)
            sqlcom.CommandType = CommandType.StoredProcedure
            sqlcom.CommandText = "VS_ST_INSPECTION"
            Dim da As New SqlDataAdapter(sqlcom)
            Dim ds As New DataSet()
    ...
    Private Sub SetFilterValue()
        Try
            ...
            Try
                For ct As Integer = 0 To CheckedComboBoxEdit2.Properties.Items.Count - 1
                    ...
                    If it.CheckState = CheckState.Checked Then
                        If sl.Length = 0 Then
                            sl = "ISNULL([HPES],'') LIKE '%" & it.Value.ToString() & "%'"
                        Else
                            sl = sl & " OR ISNULL([HPES],'') LIKE '%" & it.Value.ToString() & "%'"
                        End If
                    End If
                Next
            Catch ex As Exception
            ...
            If sl.Length = 0 Then
                sl = "ISNULL([HPES],'') LIKE '%'"
            End If
            GridView1.Columns("HPES").FilterInfo = New ColumnFilterInfo(sl)
        Catch ex As Exception
    ...
    Public Sub LoadYear()
        Try
            ...
            End If
            sqlcom.Connection = con
            sqlcom.Parameters.AddWithValue("ACTION", "LIST_YEAR")
            sqlcom.CommandType = CommandType.StoredProcedure
            sqlcom.CommandText = "VS_ST_INSPECTION"
            Dim da As New SqlDataAdapter(sqlcom)
            Dim ds As New DataSet()
            da.Fill(ds)
            cbThoigian.DataSource = ds.Tables(0)
            cbThoigian.ValueMember = "Name"
            cbThoigian.DisplayMember = "Name"
        Catch generatedExceptionName As Exception
        ...
        End Try
    End Sub
    Public Shared Function CheckExistsNetworkPath(path__1 As String) As Boolean
        If String.IsNullOrEmpty(path__1) Then
            Return False
        End If
        Dim pathRoot As String = Path.GetPathRoot(path__1)
        If String.IsNullOrEmpty(pathRoot) Then
            Return False
        End If
        Dim pinfo As New ProcessStartInfo("net", "use")
        ...
        For Each line As String In output.Split(ControlChars.Lf)
            If line.Contains(pathRoot) AndAlso line.Contains("OK") Then
    ...
    Public Sub CopyPictureToLocalPath()
        Try
            ...
            End If
            sqlcom.Connection = con
            sqlcom.Parameters.AddWithValue("ACTION", "List_PicturePath")
            sqlcom.CommandType = CommandType.StoredProcedure
            sqlcom.CommandText = "VS_ST_INSPECTION"
            Dim da As New SqlDataAdapter(sqlcom)
            Dim ds As New DataSet()
            ...
            If ds.Tables.Count > 0 Then
                ...
                If Not System.IO.Directory.Exists(Application.StartupPath + "\Images") Then
                    System.IO.Directory.CreateDirectory(Application.StartupPath + "\Images")
                End If
                For Each dr As DataRow In dt.Rows
                    If pathErr <> dr(2) Then
                        If System.IO.Directory.Exists(dr(2)) Then
                            If System.IO.File.Exists(dr(0)) Then
                                If Not System.IO.File.Exists(Application.StartupPath + "\Images\" + dr(1)) Then
                                    System.IO.File.Copy(dr(0), Application.StartupPath + "\Images\" + dr(1))
                                End If
                            End If
                        Else
                            pathErr = dr(2)
    ...
    Public Sub LoadListLocation()
        Try
            ...
            cboStatus.Columns("Location Name").Width = 200
            cboStatus.Columns("Description").Width = 120
            cboStatus.SearchMode = SearchMode.AutoFilter
...
#End Region
    ...
    Private Sub GridView1_MouseDown(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles GridView1.MouseDown
        Try
            ...
            If hi.Column.Name = "btnInCharge" Then
                ...
                'Dim p As New Point(Cursor.Position.X - 140, 500)
                'frm.Location = p
    ...
    Private Sub btnExport_Click(sender As Object, e As EventArgs) Handles btnExport.Click
        Try
            If System.IO.File.Exists(Application.StartupPath & "/Inspection.xls") Then
                System.IO.File.Delete(Application.StartupPath & "/Inspection.xls")
            End If
            ...
            GridView1.Columns(GridView1.Columns.Count - 2).Width = 200
            GridView1.Columns(GridView1.Columns.Count - 2).MaxWidth = 200
            GridView1.Columns(GridView1.Columns.Count - 2).MinWidth = 200

            GridView1.Columns(GridView1.Columns.Count - 1).Width = 200
            GridView1.Columns(GridView1.Columns.Count - 1).MaxWidth = 200
            GridView1.Columns(GridView1.Columns.Count - 1).MinWidth = 200
            ...
            GridView1.ExportToExcelOld(Application.StartupPath & "/Inspection.xls")
            'GridView1.ExportToXlsx(Application.StartupPath & "/Inspection.xls")

            If System.IO.File.Exists(Application.StartupPath & "/Inspection.xls") Then
                OpenExcel(Application.StartupPath & "/Inspection.xls")
            End If
    ...
    Private Sub cbThoigian_SelectedValueChanged(sender As Object, e As EventArgs) Handles cbThoigian.SelectedValueChanged
        GetListDataByYear()
    End Sub

    Private Sub CheckedComboBoxEdit2_Closed(sender As Object, e As ClosedEventArgs) Handles CheckedComboBoxEdit2.Closed
        SetFilterValue()
    End Sub

    Private Sub btnXFilter_Click(sender As Object, e As EventArgs) Handles btnXFilter.Click
        Try
            For ct As Integer = 0 To CheckedComboBoxEdit2.Properties.Items.Count - 1
                Dim it As DevExpress.XtraEditors.Controls.CheckedListBoxItem = CheckedComboBoxEdit2.Properties.Items(ct)
