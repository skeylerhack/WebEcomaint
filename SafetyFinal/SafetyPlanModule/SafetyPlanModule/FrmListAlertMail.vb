...
Public Class FrmListAlertMail
    Private _tbPersonInCharge As New DataTable()
    Private _tbData As New DataTable()
        ...
        Try
            For i As Integer = 0 To GridView1.RowCount - 2
                ...
                ElseIf GridView1.GetRowCellValue(i, "UserLogin") = "" Then
                    ...
                    Return False
                End If
                If IsDBNull(GridView1.GetRowCellValue(i, "Count")) Then
                    XtraMessageBox.Show(Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, "FrmListNhanVienSafety", "PHAINHAPSOLUONGBAOCAO", Commons.Modules.TypeLanguage), Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, "FrmListNhanVienSafety", "msgWarning", Commons.Modules.TypeLanguage), MessageBoxButtons.OK, MessageBoxIcon.Warning)
                    'MessageBox.Show(Me, "Must input Staffname at row " & (i + 1).ToString(), "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                    GridView1.ClearSelection()
                    GridView1.FocusedRowHandle = i
                    GridView1.SelectRow(i)
                    GridView1.ClearSelection()
                    Return False
                ElseIf GridView1.GetRowCellValue(i, "Count") <= 0 Then
                    XtraMessageBox.Show(Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, "FrmListNhanVienSafety", "PHAINHAPSOLUONGBAOCAO", Commons.Modules.TypeLanguage), Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, "FrmListNhanVienSafety", "msgWarning", Commons.Modules.TypeLanguage), MessageBoxButtons.OK, MessageBoxIcon.Warning)
                    'MessageBox.Show(Me, "Must input Staffname at row " & (i + 1).ToString(), "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                    GridView1.ClearSelection()
                    GridView1.FocusedRowHandle = i
...
#Region "Get/Save data"
    Public Sub LoadListStaff()
        Try
            ...
            cboStatus.Columns("Staff name").Width = 150
            cboStatus.Columns("Description").Width = 100
    ...
    Public Sub LoadListUserlogin()
        Try
            ...
            Dim cboStatus As New DevExpress.XtraEditors.Repository.RepositoryItemLookUpEdit()
            cboStatus.NullText = ""
            cboStatus.ValueMember = "Username"
            cboStatus.DisplayMember = "Username"
            cboStatus.DataSource = ds.Tables(0)

            cboStatus.Columns.Clear()
            cboStatus.Columns.Add(New DevExpress.XtraEditors.Controls.LookUpColumnInfo("Username"))
            cboStatus.Columns.Add(New DevExpress.XtraEditors.Controls.LookUpColumnInfo("Description"))
            cboStatus.PopupWidth = 130
            cboStatus.Columns("Username").Width = 50
    ...
    End Sub
    Public Sub LoadData()
        _tbData = New DataTable
        _tbData = GetListStaffnames()

        GridControl1.DataSource = _tbData
...
#End Region
    ...
    Private Sub GridView1_MouseDown(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles GridView1.MouseDown
        ...
        If hi.Column.Name = "InCharge" Then
            ...
            Dim p As New Point(Me.Location.X + 322, Cursor.Position.Y + 10)
            frm.Location = p
            frm.ShowDialog()
            GridView1.Focus()
            ...
            If frm._id <> -1 Then
                If GridView1.FocusedRowHandle >= 0 Then
                    GridView1.SetFocusedRowCellValue("Staffname", frm._name.ToString())
                    GridView1.SetFocusedRowCellValue("UserLogin", frm._user.ToString())
                Else
                    Dim dr As DataRow = _tbData.NewRow
                    dr(1) = frm._name.ToString()
                    dr(2) = frm._user.ToString()
                    _tbData.Rows.Add(dr)
                    _tbData.AcceptChanges()
                End If
